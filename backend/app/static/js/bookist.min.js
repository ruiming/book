...
angular.module('index').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('tags/tags_tpl.html', '<div class="tags"><span>全部标签</span><div class="tags-group" ng-repeat="oneTags in vm.allTags"><div class="tags-group-one"><p>{{oneTags.title}}</p><a class="btn btn-default" ng-repeat="tag in oneTags.tags" ui-sref="tagBooklists({tag: tag})">{{tag}}</a></div></div></div>');
